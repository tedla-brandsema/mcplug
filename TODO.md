...
MCPFS v2 is a pure MCP aggregating gateway. The v1 TODO list (native fs/git/command tools) is preserved on the `legacy/v1` branch and no longer applies.

## Deferred from the v2 plan

* [ ] Managed runtime download: auto-install `bun`/`uv` into the MCPFS data dir when a config references `npx`/`bunx`/`uvx` and the runtime is missing from PATH (opt-in convenience; commands still run verbatim by default).
...
* [ ] Per-tool aliasing / richer policy (the routing map in `internal/mcpfs/aggregate.go` is built to keep the naming policy swappable).

## Ideas

* [ ] `mcpfs ls --json` for machine-readable smoke tests.
